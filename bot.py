...
PROMO_FILE = Path(__file__).parent / "promo_buttons.json"
...
# ================== أزرار التوجيه (يديرها الأدمن) ==================
# أزرار اختيارية تطلع مع رسالة الترحيب لغير الأدمن، تودي لأي رابط خارجي
# (يوتيوب، تيك توك، إلخ). ما توقف استخدام البوت - بس زر تشوفه وتضغطه إذا حبيت.

def load_promo_buttons() -> list:
...
def save_promo_buttons(buttons: list):
    PROMO_FILE.write_text(json.dumps(buttons, ensure_ascii=False, indent=2), encoding="utf-8")


def add_promo_button(label: str, url: str):
    buttons = load_promo_buttons()
    buttons.append({"label": label, "url": url})
    save_promo_buttons(buttons)


def remove_promo_button(index: int) -> bool:
    buttons = load_promo_buttons()
...
def promo_buttons_keyboard() -> InlineKeyboardMarkup | None:
    """يبني كيبورد من أزرار التوجيه المخزّنة (زر بكل سطر). يرجّع None لو ما فيه أي زر."""
    buttons = load_promo_buttons()
    if not buttons:
        return None
...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("🔗 إدارة أزرار التوجيه", callback_data="admin_promo")],
...
def promo_admin_keyboard(buttons: list) -> InlineKeyboardMarkup:
    rows = []
    for i, b in enumerate(buttons):
        rows.append([
            InlineKeyboardButton(f"🗑 حذف: {b['label']}", callback_data=f"admin_delpromo:{i}")
        ])
    rows.append([InlineKeyboardButton("➕ إضافة زر جديد", callback_data="admin_addpromo")])
    rows.append([InlineKeyboardButton("⬅️ رجوع للقائمة", callback_data="admin_menu")])
    return InlineKeyboardMarkup(rows)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=promo_buttons_keyboard(),
    )
...
async def promo_button_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    if update.effective_user.id != ADMIN_ID:
        return False
    if not context.user_data.get("awaiting_promo_button"):
        return False

    context.user_data["awaiting_promo_button"] = False
    text = (update.message.text or "").strip()

    parts = text.rsplit(maxsplit=1)
    if len(parts) != 2 or not parts[1].startswith(("http://", "https://")):
        await update.message.reply_text(
            "الصيغة غلط. لازم تكون: التسمية ثم الرابط (يبدأ بـ http:// أو https://) بآخر السطر.\n"
            "جرب ثانية من زر ➕ إضافة زر جديد."
        )
        return True

    label, url = parts
    add_promo_button(label, url)
    buttons = load_promo_buttons()
    await update.message.reply_text(
        f"تم إضافة الزر ✅\n{label} → {url}",
        reply_markup=promo_admin_keyboard(buttons),
    )
    return True
...
async def admin_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "admin_promo":
        buttons = load_promo_buttons()
        if not buttons:
            text = "ما فيه أي زر توجيه مضاف حاليًا."
        else:
            lines = "\n".join(f"• {b['label']} → {b['url']}" for b in buttons)
            text = f"أزرار التوجيه الحالية ({len(buttons)}):\n{lines}"
        await query.edit_message_text(text, reply_markup=promo_admin_keyboard(buttons))
        return

    if action == "admin_addpromo":
        context.user_data["awaiting_promo_button"] = True
        await query.edit_message_text(
            "أرسل تسمية الزر والرابط بسطر وحد، التسمية أول شي والرابط بالآخر، مثال:\n\n"
            "📺 اشترك بالقناة الآن https://youtube.com/channel/xxxx\n\n"
            "(تقدر تحط إيموجي بالتسمية عادي)"
        )
        return

    if action.startswith("admin_delpromo:"):
        index = int(action.split(":", 1)[1])
        removed = remove_promo_button(index)
        buttons = load_promo_buttons()
        msg = "تم الحذف ✅" if removed else "ما لقيت هذا الزر."
        if buttons:
            lines = "\n".join(f"• {b['label']} → {b['url']}" for b in buttons)
            text = f"{msg}\n\nأزرار التوجيه المتبقية ({len(buttons)}):\n{lines}"
        else:
            text = f"{msg}\n\nما بقي أي زر توجيه."
        await query.edit_message_text(text, reply_markup=promo_admin_keyboard(buttons))
        return
...
async def any_message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1ج) الأدمن يرسل "تسمية + رابط" بعد زر إضافة زر توجيه جديد
    if user.id == ADMIN_ID and context.user_data.get("awaiting_promo_button"):
        await promo_button_receive(update, context)
        return
